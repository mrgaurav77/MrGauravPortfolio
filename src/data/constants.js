import thhwel from "../images/thhwel.png"
export const Bio = {
  name: "Gaurav K Kolhe",
  roles: [
    "Full Stack Developer",
    "Data Analyst",
    "Programmer",
  ],
  description:
    "Enthusiastic and analytical MCA student with a solid foundation in computer science and hands-on experience in data analytics and web technologies. Skilled in Python, Power BI, SQL, and Excel for analyzing and visualizing data, along with knowledge of front-end development using HTML, CSS, JavaScript, and ReactJS. Passionate about transforming data into actionable insights and creating efficient, user-friendly solutions. Committed to continuous learning and contributing effectively to dynamic, technology-driven teams..",
  github: "https://github.com/mrgaurav77",
  resume:
    "https://drive.google.com/file/d/1FpehDbjGALN0PTl55Iuyy5nlHc7TZFre/view?usp=drive_link",
  linkedin: "https://www.linkedin.com/in/gaurav-kolhe-0247a2257/",
  twitter: "https://x.com/gkolhe18",
  insta: "https://www.instagram.com/alkey_gaurav_vii/",
  facebook: "#",
};

export const skills = [
  {
    title: "Frontend",
    skills: [
      {
        name: "React Js",
        image:
          "data:image/svg+xml;base64,",
      },
      // {
      //   name: "Redux",
      //   image:
      //     "https://d33wubrfki0l68.cloudfront.net/0834d0215db51e91525a25acf97433051f280f2f/c30f5/img/redux.svg",
      // },
      // {
      //   name: "Next Js",
      //   image:
      //     "data:image/png;base64,",
      // },
      {
        name: "HTML",
        image: "https://www.w3.org/html/logo/badge/html5-badge-h-solo.png",
      },
      {
        name: "CSS",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/d/d5/CSS3_logo_and_wordmark.svg/1452px-CSS3_logo_and_wordmark.svg.png",
      },
      {
        name: "JavaScript",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6a/JavaScript-logo.png/800px-JavaScript-logo.png",
      },
      {
        name: "Bootstrap",
        image:
          "https://getbootstrap.com/docs/5.3/assets/brand/bootstrap-logo-shadow.png",
      },
      // {
      //   name: "Material UI",
      //   image:
      //     "data:image/png;base64,",
      // },
      // {
      //   name: "Flutter",
      //   image:
      //     "https://cdn-images-1.medium.com/max/1200/1*5-aoK8IBmXve5whBQM90GA.png",
      // },
    ],
  },
  {
    title: "Backend",
    skills: [
      {
        name: "Node Js",
        image: "https://nodejs.org/static/images/logo.svg",
      },
      // {
      //   name: "Express Js",
      //   image:
      //     "data:image/png;base64,",
      // },
      // {
      //   name: "Graph Ql",
      //   image: "https://www.vectorlogo.zone/logos/graphql/graphql-icon.svg",
      // },
      {
        name: "Python",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/python/python-original.svg",
      },
      {
        name: "MySQL",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original-wordmark.svg",
      },
      // {
      //   name: "Postgresql",
      //   image: "https://www.postgresql.org/media/img/about/press/elephant.png",
      // },
      {
        name: "MongoDB",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/mongodb/mongodb-original-wordmark.svg",
      },
      {
        name: "PHP",
        image: "data:image/png;base64,",
      },
    ],
  },
  // {
  //   title: "Android",
  //   skills: [
  //     {
  //       name: "Java",
  //       image:
  //         "https://raw.githubusercontent.com/devicons/devicon/master/icons/java/java-original.svg",
  //     },
  //     {
  //       name: "Kotlin",
  //       image:
  //         "https://www.vectorlogo.zone/logos/kotlinlang/kotlinlang-icon.svg",
  //     },
  //     {
  //       name: "XML",
  //       image:
  //         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSBMw6_RdwKQ9bDFfnKDX1iwMl4bVJEvd9PP53XuIw&s",
  //     },
  //     {
  //       name: "Android Studio",
  //       image:
  //         "https://developer.android.com/static/studio/images/new-studio-logo-1_1920.png",
  //     },
  //   ],
  // },
  {
    title: "Data Analysis",
    skills: [
      {
        name: "Excel",
        image:
          "data:image/png;base64,",
      },
      {
        name: "Python",
        image:
          "data:image/png;base64,",
      },
      {
        name: "PowerBI",
        image:
          "data:image/png;base64,",
      },
      {
        name: "Tableau",
        image:
          "data:image/png;base64,",
      },
      {
        name: "MySQL",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original-wordmark.svg",
      },
      {
        name: "Statistics",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/4/40/Fisher_iris_versicolor_sepalwidth.svg/250px-Fisher_iris_versicolor_sepalwidth.svg.png",
      },
    ],
  },
  {
    title: "Others",
    skills: [
      {
        name: "Git",
        image:
          "data:image/png;base64,",
      },
      {
        name: "GitHub",
        image:
          "https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png",
      },
      // {
      //   name: "Docker",
      //   image:
      //     "https://raw.githubusercontent.com/devicons/devicon/master/icons/docker/docker-original-wordmark.svg",
      // },
      {
        name: "Netlify",
        image:
          "data:image/png;base64,",
      },
      {
        name: "VS Code",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/9/9a/Visual_Studio_Code_1.35_icon.svg/512px-Visual_Studio_Code_1.35_icon.svg.png?20210804221519",
      },
      // {
      //   name: "Postman",
      //   image:
      //     "https://camo.githubusercontent.com/93b32389bf746009ca2370de7fe06c3b5146f4c99d99df65994f9ced0ba41685/68747470733a2f2f7777772e766563746f726c6f676f2e7a6f6e652f6c6f676f732f676574706f73746d616e2f676574706f73746d616e2d69636f6e2e737667",
      // },
      // {
      //   name: "Adobe XD",
      //   image:
      //     "https://camo.githubusercontent.com/c205ecbe12500177d102169d97bc1c17c545155fdf5ec78c08d54ac53e5b38c1/68747470733a2f2f63646e2e776f726c64766563746f726c6f676f2e636f6d2f6c6f676f732f61646f62652d78642e737667",
      // },
      // {
      //   name: "Figma",
      //   image:
      //     "https://camo.githubusercontent.com/ed93c2b000a76ceaad1503e7eb9356591b885227e82a36a005b9d3498b303ba5/68747470733a2f2f7777772e766563746f726c6f676f2e7a6f6e652f6c6f676f732f6669676d612f6669676d612d69636f6e2e737667",
      // },
    ],
  },
];

export const experiences = [
  // {
  //   id: 0,
  //   img: "https://firebasestorage.googleapis.com/v0/b/flexi-coding.appspot.com/o/flipr.jpeg?alt=media&token=1d72532a-45eb-4c1a-a81a-c9bed9fec543",
  //   role: "DevOps & Fullstack Engineering Intern",
  //   company: "Flipr Inovations Pvt. Ltd.",
  //   date: "Aug 2023 - July 2023",
  //   desc: "Working on Flipr Platforms, managing DevOps, and streamlining the process with automation.",
  //   skills: [
  //     "Docker",
  //     "Terraform",
  //     "AWS",
  //     "EC2",
  //     "Portainer",
  //     "Nginx",
  //     "JavaScript",
  //     "TypeScript",
  //     "Node Js",
  //     " Next Js",
  //   ],
  //   doc: "https://media.licdn.com/dms/image/D4D2DAQFlp60ZqHuaFQ/profile-treasury-image-shrink_1280_1280/0/1691180828512?e=1692381600&v=beta&t=mM5Y_NE5EPlQhez5FAN6NLVSKcO_Ojt_9Gq3mnFGkAQ",
  // },
 
];

export const education = [
  {
    id: 0,
    img: "https://metbhujbalknowledgecity.ac.in/wp-content/uploads/2020/03/metnasik.png",
    school: "MET Institute of Management, Nashik",
    // date: "June-2025",
    grade: "Pursuing",
    desc: "Pursuing a Master’s in Computer Applications (MCA), focusing on advanced programming, database management, and data analytics. Actively involved in hands-on projects and assignments to strengthen technical and problem-solving skills. Complementing academic learning with relevant certifications and self-directed skill enhancement. Gaining expertise in software development, web technologies, and emerging computing trends. Preparing to apply knowledge effectively in real-world tech challenges and professional environments.",
    degree: "MCA - Master of Computer Applications",
  },
  {
    id: 0,
    img: "https://bamua.digitaluniversity.ac/user/pages/images/logo_unic.png",
    school: "Deogiri Institute of Engineering and Management Studies, chh.Sambhaji Nagar",
    // date: "June-2025",
    grade: "7.29 CGPA",
    desc: "I have successfully graduated with a B.Sc. in Computer Science, earning a CGPA of 7.29. Throughout my studies, I built a solid foundation in core computer science concepts and practical skills. I actively engaged in projects and hands-on assignments that strengthened my problem-solving abilities. To complement my degree, I completed relevant certifications and continued expanding my knowledge. Graduation has inspired me to keep learning, growing, and applying my skills in real-world opportunities.",
    degree: "B.Sc. in Computer Science",
  },
  {
    id: 1,
    img: "https://www.mahahsscboard.in/boardlogo.svg",
    school: "LDP jr. college, Dahegaon",
    // date: "March 2020",
    grade: "62.2%",
    desc: "I completed my class 12 high school education, where I studied Science with Computer Science.",
    degree: "Maharashtra State Board(XII)",
  },
  {
    id: 2,
    img: "https://www.mahahsscboard.in/boardlogo.svg",
    school: "Shree Pareshwar Mahavidyalaya, Palkhed",
    // date: "Apr 2018",
    grade: "84.2%",
    desc: "I completed my class 10 education, where I studied Science with Computer Application.",
    degree: "Maharashtra State Board (X)",
  },
];

export const projects = [
  {
    id: 0,
    title: "The Helping Hand - Plant Donation Platform",
    date: "Sep 2024 - Jan 2025",
    description:
      "The Helping Hand is a platform designed to prevent healthy plants from being discarded due to space constraints, relocation, or lack of care. It connects plant owners who can no longer maintain their plants with individuals willing to adopt them. Users can list plant details, upload photos, and find nearby adopters easily. The platform is completely free and promotes environmental sustainability by reducing plant waste. Ultimately, it fosters a community of plant lovers and ensures every plant finds a loving home..",
    image:
      {thhwel},
    tags: ["React Js", "MongoDb", "Node Js", "Bootstrap"],
    category: "web app",
    github: "https://github.com/mrgaurav77/The-Helping-Hand",
    webapp: "https://the-helping-hand.netlify.app/",
    member: [
      {
        name: "Gaurav Kolhe",
        img: "https://avatars.githubusercontent.com/u/105879073?v=4",
        linkedin: "https://www.linkedin.com/in/gaurav-kolhe-0247a2257",
        github: "https://github.com/mrgaurav77",
      },
      {
        name: "Yash Sonone",
        img: "https://avatars.githubusercontent.com/u/132239373?v=4",
        linkedin: "https://www.linkedin.com/in/yash-s-0ab717248/", 
        github: "https://github.com/yashsonone99",
      },
    ],
  },
  
];